import { z } from "zod";
/**
 * Media Extractor - Specialized Shape Extractor for Media Objects
 * 
 * Handles comprehensive extraction of media content including:
 * - Images and pictures with formatting
 * - Video frames with playback settings
 * - Audio frames with audio properties
 * - Embedded media data and thumbnails
 * 
  ...
  canHandle(shape: any): boolean {
    try {
      ...
      if (!this.isInitialized) {
        console.warn('AsposeDriver not initialized in MediaExtractor Legacy, attempting sync check');
        return false;
      }
