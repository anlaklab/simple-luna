...
class AsposeDriverFactory {
  ...
  /**
   * Get fill types for shape formatting
   */
  async getFillTypes() {
  ...
  /**
   * Check if fill is gradient type
   */
  async isGradientFill(fillFormat) {
    await this.initialize();
  ...
  /**
   * Get shape types for shape detection
   */
  async getShapeTypes() {
    await this.initialize();
   ...
   */
  async isShapeOfType(shape, shapeType) {
    await this.initialize();
   ...
   */
  async getShapeTypeString(shape) {
    await this.initialize();
  ...
  /**
   * Check if shape is table type
   */
  async isTableShape(shape) {
    await this.initialize();
    return shape && shape.getShapeType() === this.aspose.ShapeType.Table;
  }

  /**
   * Check if shape is SmartArt type
   */
  async isSmartArtShape(shape) {
    await this.initialize();
    return shape && shape.getShapeType() === this.aspose.ShapeType.SmartArt;
  }

  /**
   * Check if shape is OLE object type
   */
  async isOleObjectShape(shape) {
    await this.initialize();
    return shape && shape.getShapeType() === this.aspose.ShapeType.OleObjectFrame;
  }

  /**
   * Check if shape is connector type
   */
  async isConnectorShape(shape) {
    await this.initialize();
  ...
  async loadPresentation(filePath) {
    ...
    try {
      ...
      this.logInfo('Presentation loaded successfully', { filePath });
      return presentation;
    } catch (error) {
      this.logError('Failed to load presentation', { filePath, error: error.message });
      throw error;
    }
  }
