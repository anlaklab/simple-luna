...
class AsposeLicenseManager {
  ...
  async _loadAndApplyLicense() {
    try {
      // Check for license content in environment variable first (preferred)
      ...
      if (licenseContent) {
        ...
        console.log('🔑 [GLOBAL] Applying license from content...');
        license.setLicense(licenseContent);
      ...
      // Fallback to license file path
      const licensePath = process.env.ASPOSE_LICENSE_PATH;
      
      if (!licensePath) {
        console.warn('⚠️ [GLOBAL] No ASPOSE_LICENSE_PATH or ASPOSE_LICENSE_CONTENT configured - using evaluation mode');
        this.licenseLoaded = false;
        return;
      }
      ...
      console.log(`🔑 [GLOBAL] Loading license from file: ${absoluteLicensePath}`);
      ...
      // Create and apply license from file
      const License = this.aspose.License;
      const license = new License();
      
      console.log('🔑 [GLOBAL] Applying license from file...');
      license.setLicense(absoluteLicensePath);
      
      this.licenseLoaded = true;
      console.log('✅ [GLOBAL] License applied successfully from file');
