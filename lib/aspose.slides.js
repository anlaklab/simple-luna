...
// ✅ ROBUST JAR LOADING: Try multiple possible locations
function findAsposeJar() {
	console.log('🔍 [ASPOSE] Starting JAR file search...');
	...
	const possiblePaths = [
		path.resolve(__dirname, "aspose-slides-25.6-nodejs.jar"),           // Current directory
	...
	console.log('🔍 [ASPOSE] Checking possible JAR locations:');
	for (let i = 0; i < possiblePaths.length; i++) {
		...
		try {
			if (fs.existsSync(jarPath)) {
				...
				console.log(`✅ [ASPOSE] Found JAR! Size: ${(stats.size / 1024 / 1024).toFixed(2)}MB`);
				return jarPath;
			} else {
	...
	console.log('🔍 [ASPOSE] JAR not found in any location. Directory contents:');
	try {
		console.log(`📁 __dirname (${__dirname}):`, fs.readdirSync(__dirname));
	} catch (e) {
	...
	try {
		...
		console.log(`📁 lib directory (${libDir}):`, fs.readdirSync(libDir));
	} catch (e) {
		console.log(`❌ Cannot read lib directory: ${e.message}`);
	}
	
	try {
		const appLibDir = "/app/lib";
		console.log(`📁 /app/lib directory:`, fs.readdirSync(appLibDir));
	} catch (e) {
...
try {
	...
	console.error('❌ [ASPOSE] CRITICAL ERROR: Failed to load Aspose JAR file');
