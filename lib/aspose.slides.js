...
// ✅ ENHANCED JAVA ENVIRONMENT SETUP
console.log('🔧 [ASPOSE] Setting up Java environment for Aspose.Slides...');

// Configure Java options for proper class loading
const javaOptions = [
...
// Add Java options to the Java VM
javaOptions.forEach(option => {
	if (!java.options.includes(option)) {
		java.options.push(option);
	}
});

console.log('✅ [ASPOSE] Java options configured:', java.options);

// Only set asyncOptions if not already configured
...
console.log('✅ [ASPOSE] Java async options configured');

// ✅ ENHANCED JAR LOADING: Try multiple possible locations with detailed logging
function findAsposeJar() {
	console.log('🔍 [ASPOSE] Starting comprehensive JAR file search...');
	console.log('🔍 [ASPOSE] Current __dirname:', __dirname);
	console.log('🔍 [ASPOSE] Current working directory:', process.cwd());
	console.log('🔍 [ASPOSE] Node.js version:', process.version);
	console.log('🔍 [ASPOSE] Platform:', os.platform());
	
	// Possible JAR locations in order of preference
	const possiblePaths = [
		path.resolve(__dirname, "aspose-slides-25.6-nodejs.jar"),           // Current lib directory
		path.resolve("/app/lib", "aspose-slides-25.6-nodejs.jar"),          // Docker absolute path
		path.resolve(process.cwd(), "lib", "aspose-slides-25.6-nodejs.jar"), // Project lib folder
		path.resolve(__dirname, "..", "lib", "aspose-slides-25.6-nodejs.jar"), // Parent lib folder
		path.resolve(process.cwd(), "aspose-slides-25.6-nodejs.jar"),        // Root directory
		path.resolve(__dirname, "..", "aspose-slides-25.6-nodejs.jar"),      // Parent directory
		path.resolve("/app", "aspose-slides-25.6-nodejs.jar"),               // App root
	];
	
	console.log('🔍 [ASPOSE] Checking JAR file locations:');
	for (let i = 0; i < possiblePaths.length; i++) {
		const jarPath = possiblePaths[i];
		console.log(`🔍 [ASPOSE] ${i + 1}. Checking: ${jarPath}`);
		
		try {
			if (fs.existsSync(jarPath)) {
				const stats = fs.statSync(jarPath);
				const sizeInMB = (stats.size / 1024 / 1024).toFixed(2);
				console.log(`✅ [ASPOSE] Found JAR! Size: ${sizeInMB}MB`);
				console.log(`✅ [ASPOSE] JAR permissions: ${stats.mode.toString(8)}`);
				console.log(`✅ [ASPOSE] JAR modified: ${stats.mtime.toISOString()}`);
				
				// Verify JAR file is not corrupted
				if (stats.size > 1024 * 1024) { // Must be at least 1MB
					console.log(`✅ [ASPOSE] JAR file size validation passed (${sizeInMB}MB)`);
					return jarPath;
				} else {
					console.log(`❌ [ASPOSE] JAR file too small (${sizeInMB}MB), possibly corrupted`);
				}
			} else {
				console.log(`❌ [ASPOSE] Not found: ${jarPath}`);
			}
		} catch (error) {
			console.log(`❌ [ASPOSE] Error checking ${jarPath}: ${error.message}`);
		}
	}
	
	// If no JAR found, list directory contents for debugging
	console.log('🔍 [ASPOSE] JAR not found in any location. Debugging directory contents:');
	...
	try {
		...
		files.forEach(file => {
			try {
				...
				console.log(`  - ${file} (${stats.isDirectory() ? 'DIR' : 'FILE'}) - ${(stats.size / 1024).toFixed(1)}KB`);
			} catch (e) {
	...
	// Debug /app/lib directory
	try {
		const appLibDir = "/app/lib";
		console.log(`📁 App lib directory (${appLibDir}):`);
		const files = fs.readdirSync(appLibDir);
		files.forEach(file => {
			try {
				const filePath = path.join(appLibDir, file);
				const stats = fs.statSync(filePath);
				console.log(`  - ${file} (${stats.isDirectory() ? 'DIR' : 'FILE'}) - ${(stats.size / 1024).toFixed(1)}KB`);
			} catch (e) {
				console.log(`  - ${file} (ERROR: ${e.message})`);
		...
		console.log(`❌ Cannot read /app/lib directory: ${e.message}`);
	}
	
	// Look for any JAR files in the system
	console.log('🔍 [ASPOSE] Searching for any JAR files containing "aspose"...');
	[__dirname, '/app/lib', '/app', process.cwd()].forEach(dir => {
		try {
			const files = fs.readdirSync(dir);
			files.forEach(file => {
				if (file.toLowerCase().includes('aspose') && file.toLowerCase().includes('.jar')) {
					console.log(`📦 Found potential JAR: ${path.join(dir, file)}`);
				}
			});
		} catch (e) {
	...
	throw new Error(`❌ Aspose JAR file not found in any of the expected locations. Please ensure aspose-slides-25.6-nodejs.jar is available.`);
}

// ✅ ENHANCED JAR LOADING WITH RETRY LOGIC
let jarPath;
let jarLoadAttempts = 0;
const maxRetries = 3;

while (jarLoadAttempts < maxRetries) {
	try {
		console.log(`🚀 [ASPOSE] JAR loading attempt ${jarLoadAttempts + 1}/${maxRetries}`);
		
		jarPath = findAsposeJar();
		console.log('🚀 [ASPOSE] Adding JAR to classpath:', jarPath);
		
		// Clear any existing classpath entries for this JAR
		java.classpath = java.classpath.filter(cp => !cp.includes('aspose-slides'));
		
		// Add JAR to classpath
		java.classpath.push(jarPath);
		
		console.log('✅ [ASPOSE] JAR successfully added to Java classpath');
		console.log('🔍 [ASPOSE] Current classpath entries:', java.classpath.length);
		
		// Verify JAR is in classpath
		if (java.classpath.includes(jarPath)) {
			console.log('✅ [ASPOSE] JAR verified in classpath');
			break;
		} else {
			throw new Error('JAR not found in classpath after addition');
		}
		
	} catch (error) {
		jarLoadAttempts++;
		console.error(`❌ [ASPOSE] JAR loading attempt ${jarLoadAttempts} failed: ${error.message}`);
		
		if (jarLoadAttempts >= maxRetries) {
			console.error('❌ [ASPOSE] CRITICAL ERROR: Failed to load Aspose JAR file after all retries');
			console.error('❌ [ASPOSE] Error details:', error.message);
			console.error('❌ [ASPOSE] This will cause ClassNotFoundException errors when using Aspose.Slides');
			
			// Don't throw here to allow the module to load, but log the error clearly
			console.error('❌ [ASPOSE] Continuing without JAR - Aspose functionality will be limited');
		} else {
			console.log(`⏳ [ASPOSE] Retrying JAR loading in 1 second...`);
...
// ✅ ENHANCED JAVA ENVIRONMENT VERIFICATION
console.log('🧪 [ASPOSE] Verifying Java environment...');
try {
	// Test Java runtime
	console.log('🔍 [ASPOSE] Java options:', java.options);
	console.log('🔍 [ASPOSE] Java classpath entries:', java.classpath.length);
	console.log('🔍 [ASPOSE] Java async options:', java.asyncOptions);
	
	// Test basic Java functionality
	const javaLang = java.import('java.lang.System');
	if (javaLang) {
		console.log('✅ [ASPOSE] Java System class accessible');
		try {
			const javaVersion = javaLang.getProperty('java.version');
			console.log(`✅ [ASPOSE] Java version: ${javaVersion}`);
		} catch (e) {
			console.log('⚠️ [ASPOSE] Could not get Java version:', e.message);
		}
	}
} catch (error) {
	console.error('❌ [ASPOSE] Java environment verification failed:', error.message);
}

console.log('🎉 [ASPOSE] Aspose.Slides library initialization completed');

// ✅ CRITICAL: TEST JAVA CLASS LOADING BEFORE EXPORTS
console.log('🧪 [ASPOSE] Testing Java class loading before exports...');
let javaClassLoadingSuccess = false;

try {
	// Test if we can import basic Java classes first
	console.log('🔍 [ASPOSE] Testing java.lang.String...');
	const JavaString = java.import('java.lang.String');
	if (JavaString) {
		console.log('✅ [ASPOSE] java.lang.String imported successfully');
		
		// Test if we can create a simple Java object
		const testString = java.newInstanceSync('java.lang.String', 'test');
		if (testString) {
			console.log('✅ [ASPOSE] Basic Java object creation successful');
			
			// Test if we can access Aspose classes
			console.log('🔍 [ASPOSE] Testing com.aspose.slides.AdjustValue...');
			const AdjustValue = java.import('com.aspose.slides.AdjustValue');
			if (AdjustValue) {
				console.log('✅ [ASPOSE] com.aspose.slides.AdjustValue imported successfully');
				javaClassLoadingSuccess = true;
			} else {
	...
	console.error('❌ [ASPOSE] Java class loading test failed:', error.message);
	console.error('❌ [ASPOSE] This explains why Aspose.Slides is not working');
	console.error('❌ [ASPOSE] Will continue loading but with limited functionality');
}

if (javaClassLoadingSuccess) {
	console.log('🎉 [ASPOSE] Java class loading test passed - proceeding with full exports');
} else {
	console.log('⚠️ [ASPOSE] Java class loading test failed - exports will be limited');
}
