...
export class VideoAssetExtractor implements VideoExtractor {
  ...
  private async extractVideosFromSlide(
    ...
    try {
      ...
      for (let shapeIndex = 0; shapeIndex < shapeCount; shapeIndex++) {
        ...
        // Check if shape is a video frame
        if (this.isVideoFrame(shape)) {
          const videoAsset = await this.extractFromVideoFrame(shape, slideIndex);
          if (videoAsset) assets.push(videoAsset);
        }
        
        // Check for grouped shapes that might contain videos
  ...
  async extractFromVideoFrame(videoFrame: any, slideIndex: number): Promise<AssetResult | null> {
    try {
      // Check if this is actually a video frame
      if (!this.isVideoFrame(videoFrame)) {
        return null;
      }
  ...
  validateAsset(asset: any): boolean {
    try {
      return this.isVideoFrame(asset) && 
             this.getVideoData(asset) &&
  ...
  private isVideoFrame(shape: any): boolean {
    try {
      // Check if shape has video properties
      return shape && (
        shape.getShapeType() === aspose.ShapeType.VideoFrame ||
        (shape.hasVideo && shape.hasVideo()) ||
        (shape.getVideoFormat && shape.getVideoFormat())
      );
    } catch {
      return false;
    }
